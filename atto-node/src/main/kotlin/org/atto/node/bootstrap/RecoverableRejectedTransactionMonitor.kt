...
class RecoverableRejectedTransactionMonitor(
    ...
    @EventListener
    fun process(message: InboundNetworkMessage<VotePush>) {
        val hashVote = message.payload.hashVote

        if (!hashVote.vote.isFinal() || voteValidator.validate(hashVote) != null) {
            return
        }
        ...
        scope.launch {
            ...
            val event = TransactionVoted(transaction, weighter.rejectionReason)
