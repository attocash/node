...
class BootstrapService(
    ...
    private suspend fun resolve(transaction: Transaction, sendVotedEvent: Boolean = true) {
        val rejectionReason = transactionValidator.validate(transaction)
        ...
        if (rejectionReason == null) {
            ...
            eventPublisher.publish(TransactionResolved(confirmedTransaction))
            return
        }

        if (sendVotedEvent) {
            eventPublisher.publish(TransactionVoted(transaction, rejectionReason))
        }
    }
}